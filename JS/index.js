...
if(window.location.pathname == "/examJs5/search.html"){
...
document.querySelectorAll('[data-area]').forEach(function (el) {
    el.addEventListener('click', function () {
        console.log(el.getAttribute('data-area'));
        goToAreaMeals(el.getAttribute('data-area'));
    });
});

async function goToAreaMeals(input){
    let areaMealsInfo = await getApi(`filter.php?a=${input}`)
